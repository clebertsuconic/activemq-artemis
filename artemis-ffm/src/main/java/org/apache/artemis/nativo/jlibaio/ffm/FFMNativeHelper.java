...
import static org.apache.artemis.nativo.jlibaio.ffm.FFMHandles.FCNTL_HANDLE;
...
public class FFMNativeHelper<Callback extends SubmitInfo> {
            ...
            while (running) {
               ...
               for (int i = 0; i < result; i++) {
                  ...
                  if (useFdatasync && lastFile != fd) {
                     lastFile = fd;
                     try {
                        if (openedWithODirect(fd)) {
                           if (logger.isTraceEnabled()) {
                              logger.trace("blockedPoll: fdatasync is not needed, as file fd={} is opened with O_DIRECT", fd);
                           }
                        } else {
                           fdatasync(arena, fd);
                        }
   ...
   private boolean openedWithODirect(int fd) throws Throwable {
      final int F_GETFL = 3;
      int flag = (int) FCNTL_HANDLE.invoke(fd, F_GETFL, 0L);
      return (flag & O_DIRECT) != 0;
   }
