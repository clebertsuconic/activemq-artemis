...
public class RecoverMessages extends DBOption {
   ...
   @Option(name = "--target", description = "Output folder container the new journal with all the generated messages", required = false)
   ...
   public Object execute(ActionContext context) throws Exception {
      ...
      try {
         if (configuration.isJDBC()) {
            ...
            recover(configuration, new File(getJournal()), journalOutput, reclaimed);
         }
      } catch (Exception e) {
   ...
   public static void recover(Configuration configuration, File journal, File journalOutput, boolean reclaimed) throws Exception {
      ...
      for (JournalFile file : files) {
         ...
         JournalImpl.readJournalFile(messagesFF, file, new JournalReaderCallback() {
            ...
            public void onReadUpdateRecord(RecordInfo info) throws Exception {
               if (userRecordsOfInterest.contains(info.getUserRecordType())) {
                  try {
                     targetJournal.appendUpdateRecord(info.id, info.userRecordType, info.data, false);
                  } catch (Exception e) {
                     System.out.println("no add record for " + info.id + ", ignoring it");
                  }
               }
            }
