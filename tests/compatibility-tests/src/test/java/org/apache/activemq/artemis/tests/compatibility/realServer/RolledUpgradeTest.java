...
public class RolledUpgradeTest extends CompatBaseTest {
   ...
   private static final String TWO_THIRTY_ZIP = "./target/old-releases/apache-artemis-2.30.0-bin.zip";
   private static final String TWO_THIRTY_FIVE_ZIP = "./target/old-releases/apache-artemis-2.35.0-bin.zip";
   ...
   private static void newServerCLI(File home, String location, int serverID, String replicaGroupName, boolean live, int [] connectedNodes) throws Exception {
      File serverLocation = getFileServerLocation(location);
      deleteDirectory(serverLocation);
      HelperCreate cliCreateServer = new HelperCreate();
      cliCreateServer.setPortOffset(getPortOffeset(serverID));
      ...
      cliCreateServer.setArgs("--host", getHost(serverID), "--clustered", "--static-cluster", clusterList.toString(), "--name", "main0", "--replicated", "--replica-group-name", replicaGroupName, "--no-amqp-acceptor", "--no-mqtt-acceptor", "--no-hornetq-acceptor", "--no-stomp-acceptor");
   ...
   private static void newServer(File homeLocation, String location, int serverID, String replicaGroupName, boolean live, int [] connectedNodes) throws Exception {
      File serverLocation = getFileServerLocation(location);
      ...
      if (live) {
         boolean replacedMaster = FileUtil.findReplace(brokerXml, "</primary>",
     ...
     "   <group-name>" + replicaGroupName + "</group-name>\n" +
         ...
         Assertions.assertTrue(replacedMaster, "couldn't find either master or primary on broker.xml");
      } else {
         ...
         replacedSlave |= FileUtil.findReplace(brokerXml, "<backup/>",
                                                      "<backup>\n" +
                                                         ...
                                                         "              <quorum-size>2</quorum-size>\n" +
                                                         "            </backup>");
         Assertions.assertTrue(replacedSlave, "couldn't find slave on backup to replace on broker.xml");
      }
   ...
   public static void createServers(File sourceHome) throws Exception {
      ...
      newServerCLI(sourceHome, LIVE_0 + "_cli", 0, "live0", true, new int[] {1, 2, 3, 4, 5});
      newServerCLI(sourceHome, LIVE_1 + "_cli", 1, "live1", true, new int[] {0, 2, 3, 4, 5});
      newServerCLI(sourceHome, LIVE_2 + "_cli", 2, "live2", true, new int[] {0, 1, 3, 4, 5});
      newServerCLI(sourceHome, BKP_0 + "_cli", 3, "live0", false, new int[] {0, 1, 2, 4, 5});
      newServerCLI(sourceHome, BKP_1 + "_cli", 4, "live1", false, new int[] {0, 1, 2, 3, 5});
      newServerCLI(sourceHome, BKP_2 + "_cli", 5, "live2", false, new int[] {0, 1, 2, 3, 4});
   }
   ...
   public void testRolledUpgrade_2_35() throws Exception {
      testRolledOnZipDistribution(TWO_THIRTY_FIVE_ZIP, "apache-artemis-2.35.0");
   }

   private void testRolledOnZipDistribution(String zip, String destination) throws Exception {
