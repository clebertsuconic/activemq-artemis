...
public class FailoverPriorityTest extends OpenwireArtemisBaseTest {
   ...
   private void restart(boolean primary, int primaryID, int secondaryID) throws Exception {
      ...
      if (primary) {
         Configuration config = createConfig(primaryID);
         ...
         Assert.assertTrue(servers[primaryID].waitClusterForming(100, TimeUnit.MILLISECONDS, 20, 2));
         Assert.assertTrue(servers[secondaryID].waitClusterForming(100, TimeUnit.MILLISECONDS, 20, 2));
      }
      else {
         Configuration config = createConfig(secondaryID);
