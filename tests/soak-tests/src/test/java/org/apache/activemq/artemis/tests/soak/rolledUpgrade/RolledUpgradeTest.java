...
public class RolledUpgradeTest extends SoakTestBase {
   ...
   private static void newServerCLI(String home, String location, int serverID, String replicaGroupName, boolean live, int [] connectedNodes) throws Exception {
   ...
   private static void newServer(String homeLocation, String location, int serverID, String replicaGroupName, boolean live, int [] connectedNodes) throws Exception {
      ...
      if (live) {
         assertTrue(FileUtil.findReplace(brokerXml, "</master>",
     "   <group-name>" + replicaGroupName + "</group-name>\n" +
            "               <check-for-live-server>true</check-for-live-server>\n" +
            "               <quorum-size>2</quorum-size>\n" +
            "            </master>"));
      } else {
         assertTrue(FileUtil.findReplace(brokerXml, "<slave/>",
                                         "<slave>\n" +
                                                 "              <group-name>" + replicaGroupName + "</group-name>\n" +
   ...
   public void testJustUpgrade() throws Exception {
      upgrade(HelperBase.getHome(), getFileServerLocation(LIVE_0));
      upgrade(HelperBase.getHome(), getFileServerLocation(LIVE_1));
      upgrade(HelperBase.getHome(), getFileServerLocation(LIVE_2));

      upgrade(HelperBase.getHome(), getFileServerLocation(BKP_0));
      upgrade(HelperBase.getHome(), getFileServerLocation(BKP_1));
      upgrade(HelperBase.getHome(), getFileServerLocation(BKP_2));
   }
