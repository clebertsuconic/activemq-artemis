...
public class InsertReferencesStatement extends BatchableStatement<MessageReferenceData> {
   ...
   public InsertReferencesStatement(Connection connection, JDBCConnectionProvider connectionProvider, DatabaseStorageConfiguration databaseStorageConfiguration, int expectedSize) throws SQLException {
   ...
   private static String getSQL(JDBCConnectionProvider connectionProvider, DatabaseStorageConfiguration databaseStorageConfiguration) {
      ...
      String sql = connectionProvider.getSQLProvider().getInsertPDBReferences(tableName);
      if (sql == null) {
         sql = "INSERT INTO " + tableName + " (MESSAGE_ID, QUEUE_ID, TX_ID) VALUES (?,?,?)";
      }
      return sql;
   }
