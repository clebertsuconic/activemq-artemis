...
import org.apache.activemq.artemis.core.persistence.impl.newdatabase.dbdata.AddressData;
...
import org.apache.activemq.artemis.core.persistence.impl.newdatabase.dbdata.DeleteReferenceData;
...
import org.apache.activemq.artemis.core.persistence.impl.newdatabase.dbdata.TXDone;
...
public class DataManager extends ActiveMQScheduledComponent {
   ...
   public DataManager(ScheduledExecutorService scheduledExecutorService,
      ...
      for (int i = 0; i < NUMBER_OF_CONNECTIONS; i++) {
         DataWorker worker =  new DataWorker(this::workerDone, connectionProvider, databaseConfiguration, batchSize, "worker " + i);
   ...
   private void flushData(List<DBData> dbData, StorageTX storageTX) {
      synchronized (pendingData) {
         ...
         pendingData.add(new TXDone((NewDatabaseStoreTX) storageTX));
      }
      delay();
   }
   ...
   private NewDatabaseStoreTX castTX(StorageTX storageTX) {
      return (NewDatabaseStoreTX) storageTX;
   }
   ...
   public void storeQueue(StorageTX storageTX, long addressId, long id,
                          String name,
                          String filter,
                          RoutingType routingType, IOCompletion callback) {

      castTX(storageTX).addData(new QueueData(addressId, id, name, filter, routingType == RoutingType.MULTICAST, routingType == RoutingType.ANYCAST, callback));
   }
   ...
   public void storeAddressInfo(StorageTX storageTX, AddressInfo addressInfo, IOCompletion callback) {
      castTX(storageTX).addData(new AddressData(addressInfo,  callback));
   }
