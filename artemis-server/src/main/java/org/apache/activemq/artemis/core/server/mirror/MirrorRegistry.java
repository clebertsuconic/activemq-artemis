...
public class MirrorRegistry {
   ...
   private static final ThreadLocal<MirrorController> THREAD_REGISTRY = new ThreadLocal<>();

   public static void setController(MirrorController controller) {
      THREAD_REGISTRY.set(controller);
   }

   public static MirrorController getController() {
      return THREAD_REGISTRY.get();
   }
